...
st.markdown('<h1 class="main-header">🏠 Prédiction de Valeur Foncière</h1>', unsafe_allow_html=True)
st.markdown("---")
...
def load_geo_data():
    ...
    file_path = 'Liste_Departement_Ville_ValeursFoncieres-2023_filtree.csv'
...
def load_model_and_encoders():
    ...
    model_path = 'model_cb.pkl'  # CORRECTION: Nom de fichier cohérent
...
if not geo_df.empty and model is not None:
    ...
    if geo_df.empty:
        ...
        st.info("Vérifiez la présence du fichier 'Liste_Departement_Ville_ValeursFoncieres-2023_filtree.csv'")
    
    if model is None:
        st.error("❌ Modèle non disponible")
        st.info("Vérifiez la présence du fichier 'model_cb.pkl'")
